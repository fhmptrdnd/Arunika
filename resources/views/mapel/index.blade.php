    ...
    <div class="max-w-5xl mx-auto flex flex-col min-h-[75vh] relative z-20 py-10 px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-pop-in delay-100">
            @foreach ($subjects as $item)
                @php
                    $kunciPencarian = $item['slug'];

                    $levelSelesai = $progresSiswa[$kunciPencarian] ?? 0;

                    $totalLevel = 8;
                    $persentase = min(100, round(($levelSelesai / $totalLevel) * 100));
                @endphp

                <a href="{{ route('mapel.levels', ['slug' => $item['slug'], 'kelas' => $kelas]) }}"
                    ...
                    <div class="w-32 h-32 {{ $item['color'] }} rounded-full flex items-center justify-center text-6xl shadow-inner mb-6 group-hover:scale-110 transition-transform duration-300">
                        {{ $item['icon'] }}
                    </div>
                    ...
                    <div class="w-full bg-gray-100 rounded-full h-3 mt-4 overflow-hidden shadow-inner">
                        <div class="bg-yellow h-3 rounded-full transition-all duration-1000" style="width: {{ $persentase }}%"></div>
                    </div>

                    @if($levelSelesai > 0)
                        <p class="text-sm text-orange mt-2 font-bold">
                            Selesai {{ $levelSelesai }} / {{ $totalLevel }} Level
                        </p>
                    @else
