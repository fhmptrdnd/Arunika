...
<div class="max-w-5xl mx-auto flex flex-col min-h-[75vh] relative z-20 py-10 px-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-pop-in delay-100">
        ...
        <a href="{{ route('mapel.levels', ['slug' => $mapel['slug']]) }}" class="bg-white rounded-[40px] p-8 shadow-xl hover:-translate-y-3 hover:shadow-2xl transition-all duration-300 flex flex-col items-center text-center no-underline group border-b-8 border-transparent hover:border-[#12A0D7]">

            <div class="w-32 h-32 {{ $mapel['color'] }} rounded-full flex items-center justify-center text-6xl shadow-inner mb-6 group-hover:scale-110 transition-transform duration-300">
                {{ $mapel['icon'] }}
            </div>
