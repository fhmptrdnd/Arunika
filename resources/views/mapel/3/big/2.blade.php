...
<html lang="id">
  <head>
    ...
    <style>
      ...
      .spyglass-lens {
        ...
        width: 100px;
        height: 100px;
        border: 8px solid #3b82f6; /* Warna bingkai */
        border-radius: 50%;
      ...
      #hand-cursor.focused .spyglass-lens {
        border-color: #f59e0b; /* Berubah amber */
  ...
  <body
    class="bg-gradient-to-b from-sky-300 via-sky-200 to-green-300 h-screen w-full relative overflow-hidden"
  >
    ...
    <div class="absolute inset-0 pointer-events-none z-0">
      ...
      <div
        class="absolute top-20 left-[-10%] animate-drift-fast opacity-70 scale-75"
      >
      ...
      <div class="town-road road-vertical"><div class="road-dash-v"></div></div>
      <div
    ...
    <!-- --- TOP BAR --- -->
    <div
      class="relative z-20 flex justify-between items-start p-4 md:p-6 w-full pointer-events-none"
    >
      <!-- Info Pemain -->
      <div
      ...
      >
        ...
        <div>
          <h2
            class="font-black text-xl text-indigo-900 tracking-wide drop-shadow-sm"
          >
            Explorer Level 3
          </h2>
      ...
      <!-- Bar XP -->
      <div
        class="hidden md:flex flex-col items-center pt-2 pointer-events-auto"
      >
        <div
          class="bg-white/90 backdrop-blur-md px-6 py-2 rounded-2xl border-4 border-white shadow-lg flex flex-col items-center"
        >
          <span class="font-black text-indigo-800 text-lg mb-1" id="xp-text"
            >0 XP</span
          >
          <div
      ...
      <div class="flex items-start gap-4 pointer-events-auto">
        <div
          class="relative hidden lg:flex flex-col items-end pt-2 animate-float"
        >
          <div
        ...
        >
          <div class="absolute top-1 left-1 flex items-center gap-1 z-10">
            ...
            <span
              id="cam-status"
              class="text-[9px] text-white font-bold tracking-wider drop-shadow-md"
              >LOAD</span
            >
          </div>
          <video
            id="input_video"
            class="w-full h-full object-cover rounded-lg mt-3 scale-x-[-1]"
            autoplay
            playsinline
          ></video>
        </div>
    ...
    <div
      class="absolute inset-0 top-24 bottom-4 z-10 flex flex-col items-center pointer-events-none"
    >
      <!-- Target Kata (Misi Detektif) -->
      <div
        class="text-center shrink-0 pointer-events-auto mt-2 mb-4 animate-bounce-slight"
      >
        <div
        ...
        >
          <span
            class="text-sm font-bold text-indigo-600 uppercase tracking-widest leading-none mb-1"
            >Misi: Temukan Lokasi</span
          >
          <div class="flex items-center gap-4">
            <button
              onclick="playTargetSpeech()"
            ...
            <span
              id="target-word"
              class="font-fun text-4xl md:text-5xl text-indigo-900 tracking-wide uppercase drop-shadow-sm"
              >LIBRARY</span
            >
          </div>
        </div>
      ...
      <div
        id="town-map"
        class="relative w-full max-w-5xl flex-1 z-20 pointer-events-auto"
      >
      ...
      <!-- Indikator Progres -->
      <div
        class="absolute bottom-6 left-6 flex gap-3 pointer-events-auto bg-white/90 backdrop-blur-sm px-6 py-3 rounded-full border-4 border-white shadow-lg z-40"
        id="progress-dots"
      >
        <div
          class="w-5 h-5 bg-gray-300 rounded-full border-2 border-white shadow-sm dot"
        ></div>
        <div
          class="w-5 h-5 bg-gray-300 rounded-full border-2 border-white shadow-sm dot"
        ></div>
    ...
    <div
      id="victory-overlay"
      class="hidden fixed inset-0 z-[70] bg-indigo-900/90 backdrop-blur-md flex-col items-center justify-center pointer-events-auto"
    >
      <div
        id="victory-modal"
        class="bg-white p-8 md:p-12 rounded-[3rem] shadow-[0_0_50px_rgba(99,102,241,0.5)] border-8 border-yellow-400 flex flex-col items-center max-w-xl w-[90%] text-center scale-0 relative overflow-hidden"
      >
        <div
          class="absolute -top-10 -right-10 text-9xl opacity-10 pointer-events-none"
        >
          🎉
        </div>

        <div
          class="w-28 h-28 bg-gradient-to-tr from-indigo-400 to-blue-500 rounded-full border-4 border-white flex items-center justify-center mb-4 shadow-[0_0_30px_rgba(99,102,241,0.8)] animate-bounce relative z-10"
        >
          <i data-lucide="map" class="text-white w-14 h-14"></i>
        </div>

        <div
          id="victory-stars"
          class="flex justify-center gap-3 mb-3 relative z-10"
        ></div>

        <h1
          id="victory-title"
          class="text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-500 mb-2 drop-shadow-sm pb-1 relative z-10"
        >
          Detektif Kota Hebat!
        </h1>
        <p
          class="text-xl text-gray-600 mb-6 font-bold relative z-10"
          id="victory-subtitle"
        >
          Kamu sangat hafal lokasi-lokasinya!
        </p>

        <div class="flex flex-col gap-3 mb-8 w-full z-10">
          <div
            class="flex justify-between items-center bg-indigo-50 p-3 rounded-2xl border-2 border-indigo-100"
          >
            <span class="font-bold text-gray-600 flex items-center gap-2"
              ...
              ></i>
              Lokasi Benar (<span id="correct-count">5</span>)</span
            >
            <span class="font-black text-green-500" id="xp-answers"
              >+25 XP</span
            >
          </div>
          <div
          ...
          >
            <span class="font-bold text-gray-600 flex items-center gap-2"
              ><i data-lucide="flag" class="w-5 h-5 text-blue-500"></i> Selesai
              Level</span
            >
            <span class="font-black text-green-500">+25 XP</span>
          </div>
          <div
            id="flawless-badge"
            class="flex justify-between items-center bg-yellow-50 p-3 rounded-2xl border-2 border-yellow-200"
          >
            <span class="font-bold text-yellow-600 flex items-center gap-2"
              ><i
                data-lucide="star"
                class="w-5 h-5 text-yellow-500 fill-current"
              ></i>
              Bonus Tanpa Salah!</span
            >
            <span class="font-black text-yellow-500">+10 XP</span>
          </div>
          <div
            class="mt-2 flex justify-between items-center bg-indigo-100 p-4 rounded-2xl border-4 border-indigo-300 shadow-inner"
          >
            <span class="font-black text-indigo-900 text-lg"
              >Total XP Didapat</span
            >
            <span
              class="font-black text-indigo-600 text-3xl"
              id="earned-xp-text"
              >+60</span
            >
          </div>
        ...
        <button
          ...
          class="bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white text-xl font-black py-4 px-12 rounded-full shadow-[0_6px_0_#4338ca] hover:shadow-[0_3px_0_#4338ca] hover:translate-y-1 transition-all w-full md:w-auto z-10"
        >
          Selesai
        </button>
      </div>
    ...
    <script>
      ...
      function renderMap() {
        ...
        places.forEach((place) => {
          ...
          // Base class
          bldgWrapper.className = `building-target absolute w-36 h-40 md:w-44 md:h-48 rounded-[2.5rem] border-8 border-white/90 shadow-[0_15px_20px_rgba(0,0,0,0.2)] flex flex-col items-center justify-center bg-gradient-to-br ${place.color.replace("bg-", "from-")}-400 to-${place.color.replace("bg-", "")}-600`;
      ...
      function showVictory() {
        ...
        for (let i = 1; i <= 3; i++) {
          if (i <= stars) {
            starsContainer.innerHTML += `<i data-lucide="star" class="w-12 h-12 text-yellow-400 fill-current drop-shadow-md animate-bounce-slight" style="animation-delay: ${i * 0.1}s"></i>`;
          } else {
            starsContainer.innerHTML += `<i data-lucide="star" class="w-12 h-12 text-gray-300 fill-current drop-shadow-sm"></i>`;
          }
        }
        lucide.createIcons();
