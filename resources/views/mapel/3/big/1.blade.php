...
<html lang="id">
  <head>
    ...
    <style>
      ...
      .font-fun { font-family: 'Fredoka One', cursive; text-shadow: 2px 2px 0px rgba(0,0,0,0.1); }

      /* Animasi Latar & Elemen */
      @keyframes float { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-10px); } }
      .animate-float { animation: float 3s ease-in-out infinite; }

      @keyframes drift-slow { from { transform: translateX(-10%); } to { transform: translateX(110vw); } }
      @keyframes drift-fast { from { transform: translateX(-10%); } to { transform: translateX(110vw); } }
      .animate-drift-slow { animation: drift-slow 45s linear infinite; }
      .animate-drift-fast { animation: drift-fast 30s linear infinite; }
      ...
      .animate-pop-in { animation: pop-in 0.4s forwards cubic-bezier(0.175, 0.885, 0.32, 1.275); }
      ...
      .animate-camera-flash { animation: camera-flash 0.8s ease-out forwards; }
      ...
      .animate-polaroid { animation: polaroid-pop 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }
      ...
      .animate-float-up-fade { animation: float-up-fade 1s ease-out forwards; pointer-events: none; z-index: 100; }
      ...
      .corner-tl { top: 0; left: 0; border-top-width: 6px; border-left-width: 6px; border-top-left-radius: 12px; }
      .corner-tr { top: 0; right: 0; border-top-width: 6px; border-right-width: 6px; border-top-right-radius: 12px; }
      .corner-bl { bottom: 0; left: 0; border-bottom-width: 6px; border-left-width: 6px; border-bottom-left-radius: 12px; }
      .corner-br { bottom: 0; right: 0; border-bottom-width: 6px; border-right-width: 6px; border-bottom-right-radius: 12px; }

      /* Efek Fokus Bingkai (Target Terkunci) */
      #hand-cursor.focused .viewfinder-corner { border-color: #10b981; }
      #hand-cursor.focused { transform: translate(-50%, -50%) scale(0.9); }
  ...
  <body class="bg-gradient-to-b from-sky-300 via-sky-200 to-green-300 h-screen w-full relative overflow-hidden">
    ...
    <div id="hand-cursor" style="opacity: 0">
      ...
      <svg class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" width="60" height="60">
        <circle class="progress-ring__circle" stroke="rgba(59, 130, 246, 0.5)" stroke-width="4" fill="transparent" r="26" cx="30" cy="30" stroke-dasharray="163.3" stroke-dashoffset="163.3" id="cursor-progress"/>
      </svg>
      
      <!-- Titik fokus tengah -->
      <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-2 h-2 bg-blue-500 rounded-full" id="center-dot"></div>
    </div>

    <!-- --- SCENERY BACKGROUND (Neighborhood) --- -->
    <div class="absolute inset-0 pointer-events-none z-0">
      <div class="absolute top-10 left-[-20%] animate-drift-slow opacity-90">
        <svg width="180" height="90" viewBox="0 0 24 24" fill="white"><path d="M17.5 19c2.485 0 4.5-2.015 4.5-4.5S19.985 10 17.5 10c-.17 0-.336.01-.5.027A6.49 6.49 0 0011 5c-3.59 0-6.5 2.91-6.5 6.5 0 .238.013.473.039.704A4.5 4.5 0 005.5 21h12z" /></svg>
      </div>
      <div class="absolute top-28 left-[-10%] animate-drift-fast opacity-70 scale-75">
        <svg width="250" height="120" viewBox="0 0 24 24" fill="white"><path d="M17.5 19c2.485 0 4.5-2.015 4.5-4.5S19.985 10 17.5 10c-.17 0-.336.01-.5.027A6.49 6.49 0 0011 5c-3.59 0-6.5 2.91-6.5 6.5 0 .238.013.473.039.704A4.5 4.5 0 005.5 21h12z" /></svg>
      </div>
      
      <!-- Hutan & Rumah (Neighborhood) -->
      <div class="absolute bottom-0 w-full h-[25vh] bg-green-500 rounded-t-[100%] scale-x-150 transform translate-y-12 shadow-[inset_0_20px_50px_rgba(0,0,0,0.1)]"></div>
      <div class="absolute bottom-[-10%] w-[120%] left-[-10%] h-[35vh] bg-green-400 rounded-t-[100%] scale-x-110"></div>
    ...
    <div class="relative z-20 flex justify-between items-start p-4 md:p-6 w-full pointer-events-none">
      <!-- Info Pemain -->
      <div class="flex items-center gap-3 bg-white/80 backdrop-blur-md p-2 pr-6 rounded-full border-4 border-white shadow-lg pointer-events-auto">
        <div class="w-14 h-14 bg-blue-500 rounded-full border-2 border-white flex items-center justify-center text-3xl shadow-inner">📸</div>
        <div>
      ...
      <div class="hidden md:flex flex-col items-center pt-2 pointer-events-auto">
        <div class="bg-white/80 backdrop-blur-md px-6 py-2 rounded-2xl border-4 border-white shadow-lg flex flex-col items-center">
          ...
          <div class="w-48 h-5 bg-blue-100 rounded-full overflow-hidden shadow-inner border border-blue-200">
            <div id="xp-bar-fill" class="h-full bg-gradient-to-r from-yellow-300 to-amber-500 rounded-full transition-all duration-1000 ease-out" style="width: 0%"></div>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-start gap-4 pointer-events-auto">
        <div class="relative hidden lg:flex flex-col items-end pt-2 animate-float">
          <div class="bg-white/95 px-4 py-3 rounded-2xl rounded-br-none shadow-xl border-4 border-blue-300 mb-2 max-w-[200px]">
            <p id="owl-message" class="font-bold text-blue-800 text-sm">Arahkan kameramu ke orang yang tepat untuk memotret!</p>
          </div>
          <div class="text-6xl drop-shadow-[0_10px_10px_rgba(0,0,0,0.3)]">🦉</div>
        </div>

        <div class="bg-slate-800 p-2 rounded-xl border-4 border-slate-600 shadow-xl flex flex-col items-center w-28 h-24 relative overflow-hidden">
          <div class="absolute top-1 left-1 flex items-center gap-1 z-10">
            ...
            <span id="cam-status" class="text-[9px] text-white font-bold tracking-wider drop-shadow-md">LOAD</span>
          </div>
          <video id="input_video" class="w-full h-full object-cover rounded-lg mt-3 scale-x-[-1]" autoplay playsinline></video>
        </div>
      </div>
    </div>
    ...
    <div class="absolute inset-0 top-24 bottom-10 z-10 flex flex-col items-center pointer-events-none">
      ...
      <div class="text-center mb-4 shrink-0 pointer-events-auto mt-2">
        <div class="bg-white/90 backdrop-blur-sm px-8 py-3 rounded-[2rem] border-8 border-blue-300 shadow-[0_10px_0_#93c5fd,_0_20px_25px_rgba(0,0,0,0.3)] flex flex-col items-center gap-1">
            <span class="text-sm font-bold text-blue-600 uppercase tracking-widest">Take a photo of the:</span>
            <div class="flex items-center gap-4">
                <button onclick="playTargetSpeech()" class="bg-yellow-400 hover:bg-yellow-500 p-2 rounded-full shadow-md transition-transform hover:scale-110">
                    <i data-lucide="volume-2" class="text-white w-8 h-8"></i>
                </button>
                <span id="target-word" class="font-fun text-5xl md:text-6xl text-blue-900 tracking-wide uppercase drop-shadow-sm">FATHER</span>
            </div>
        </div>
      </div>

      <!-- Area Karakter Berdiri (Taman Bermain) -->
      <div id="characters-container" class="relative w-full max-w-4xl h-[45vh] flex justify-center items-end pb-8 gap-8 md:gap-16 pointer-events-auto mt-4">
         <!-- Karakter Di-render oleh JS -->
      </div>

      <!-- Tempat Menampilkan Hasil Jepretan (Polaroid) -->
      <div id="polaroid-display" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[60] hidden flex-col items-center">
          <div class="bg-white p-4 pb-12 rounded-lg shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col items-center transform rotate-3">
              <div id="polaroid-img" class="w-48 h-48 bg-blue-100 border border-gray-200 flex items-center justify-center text-7xl shadow-inner mb-4">
                  <!-- Icon goes here -->
              </div>
      ...
      <!-- Indikator Progres -->
      <div class="absolute bottom-6 flex gap-3 pointer-events-auto bg-white/50 backdrop-blur-sm px-6 py-3 rounded-full border-2 border-white shadow-lg" id="progress-dots">
    ...
    <!-- --- VICTORY OVERLAY --- -->
    <div id="victory-overlay" class="hidden fixed inset-0 z-[70] bg-blue-900/90 backdrop-blur-md flex-col items-center justify-center pointer-events-auto">
      <div id="victory-modal" class="bg-white p-8 md:p-12 rounded-[3rem] shadow-[0_0_50px_rgba(59,130,246,0.5)] border-8 border-yellow-400 flex flex-col items-center max-w-xl w-[90%] text-center scale-0 relative overflow-hidden">
        
        <div class="absolute -top-10 -right-10 text-9xl opacity-10 pointer-events-none">🎉</div>

        <div class="w-28 h-28 bg-gradient-to-tr from-blue-400 to-sky-400 rounded-full border-4 border-white flex items-center justify-center mb-4 shadow-[0_0_30px_rgba(56,189,248,0.8)] animate-bounce relative z-10">
          <i data-lucide="camera" class="text-white w-14 h-14"></i>
        </div>

        <div id="victory-stars" class="flex justify-center gap-3 mb-3 relative z-10"></div>

        <h1 id="victory-title" class="text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-sky-500 mb-2 drop-shadow-sm pb-1 relative z-10">Photo Album Complete!</h1>
        <p class="text-xl text-gray-600 mb-6 font-bold relative z-10" id="victory-subtitle">Kamu fotografer yang hebat!</p>
        ...
        <div class="flex flex-col gap-3 mb-8 w-full z-10">
          <div class="flex justify-between items-center bg-blue-50 p-3 rounded-2xl border-2 border-blue-100">
            <span class="font-bold text-gray-600 flex items-center gap-2"><i data-lucide="check-circle-2" class="w-5 h-5 text-green-500"></i> Foto Benar (<span id="correct-count">5</span>)</span>
            <span class="font-black text-green-500" id="xp-answers">+25 XP</span>
          </div>
          <div class="flex justify-between items-center bg-blue-50 p-3 rounded-2xl border-2 border-blue-100">
            <span class="font-bold text-gray-600 flex items-center gap-2"><i data-lucide="flag" class="w-5 h-5 text-blue-500"></i> Selesai Level</span>
            <span class="font-black text-green-500">+20 XP</span>
          </div>
          <div id="flawless-badge" class="flex justify-between items-center bg-yellow-50 p-3 rounded-2xl border-2 border-yellow-200">
            <span class="font-bold text-yellow-600 flex items-center gap-2"><i data-lucide="star" class="w-5 h-5 text-yellow-500 fill-current"></i> Bonus Tanpa Salah!</span>
            <span class="font-black text-yellow-500">+10 XP</span>
          </div>
          <div class="mt-2 flex justify-between items-center bg-blue-100 p-4 rounded-2xl border-4 border-blue-300 shadow-inner">
            <span class="font-black text-blue-900 text-lg">Total XP Didapat</span>
            <span class="font-black text-blue-600 text-3xl" id="earned-xp-text">+55</span>
          </div>
        </div>

        <button onclick="window.location.reload()" class="bg-gradient-to-r from-blue-500 to-sky-500 hover:from-blue-600 hover:to-sky-600 text-white text-xl font-black py-4 px-12 rounded-full shadow-[0_6px_0_#1e3a8a] hover:shadow-[0_3px_0_#1e3a8a] hover:translate-y-1 transition-all w-full md:w-auto z-10">
          Lanjut ke Level 2
        </button>
      </div>
    </div>
    ...
    <script>
      ...
      const rounds = [
        { 
          targetWord: "FATHER", 
          options: [
            { id: "father", text: "Father", icon: "👨", color: "bg-blue-200" },
            { id: "brother", text: "Brother", icon: "👦", color: "bg-green-200" },
            { id: "mother", text: "Mother", icon: "👩", color: "bg-pink-200" }
          ],
          correct: "father"
        },
        { 
          ...
          options: [
            { id: "mother", text: "Mother", icon: "👩", color: "bg-pink-200" },
            { id: "sister", text: "Sister", icon: "👧", color: "bg-rose-200" },
            { id: "teacher", text: "Teacher", icon: "👨‍🏫", color: "bg-amber-200" }
          ],
          correct: "mother"
        },
        { 
          targetWord: "BROTHER", 
          options: [
            { id: "friend", text: "Friend", icon: "🤝", color: "bg-emerald-200" },
            { id: "brother", text: "Brother", icon: "👦", color: "bg-green-200" },
            { id: "father", text: "Father", icon: "👨", color: "bg-blue-200" }
          ],
          correct: "brother"
        ...
        { 
          ...
          options: [
            { id: "sister", text: "Sister", icon: "👧", color: "bg-rose-200" },
            { id: "mother", text: "Mother", icon: "👩", color: "bg-pink-200" },
            { id: "teacher", text: "Teacher", icon: "👩‍🏫", color: "bg-indigo-200" }
          ],
          correct: "sister"
        },
        { 
      ...
      function renderRound() {
        ...
        shuffledOptions.forEach((opt, index) => {
          ...
          charBox.className = `character-target relative w-32 h-40 md:w-40 md:h-52 rounded-t-[3rem] rounded-b-2xl border-8 border-white shadow-[0_10px_15px_rgba(0,0,0,0.2)] flex items-center justify-center text-7xl md:text-8xl transition-all duration-300 ${opt.color}`;
          
          const grass = document.createElement("div");
          grass.className = "absolute -bottom-6 w-full h-8 bg-green-600 rounded-full blur-[2px] opacity-50 z-[-1]";

          charBox.innerHTML = `<span class="drop-shadow-lg pointer-events-none">${opt.icon}</span>`;
          
          const nameTag = document.createElement("div");
          nameTag.className = "absolute -bottom-4 bg-white px-4 py-1 rounded-full border-2 border-gray-200 font-bold text-gray-700 text-sm shadow-sm";
      ...
      function handleSnapshot(selectedId) {
        ...
        if (selectedId === round.correct) {
          ...
          setTimeout(() => {
              ...
              polaroidImg.className = `w-48 h-48 border border-gray-200 flex items-center justify-center text-7xl shadow-inner mb-4 ${correctOpt.color}`;
              ...
              confetti({ particleCount: 50, spread: 80, origin: { x: 0.5, y: 0.5 }, colors: ['#facc15', '#f472b6', '#38bdf8'] });
          ...
          setTimeout(() => {
              ...
              if (lostXP > 0) {
                  ...
                  xpText.className = "absolute text-red-500 font-black text-4xl drop-shadow-md animate-float-up-fade z-50";
      ...
      function updateProgressDots() {
        document.querySelectorAll(".dot").forEach((dot, index) => {
          if (index < currentRound) {
            ...
            dot.className = "w-4 h-4 bg-yellow-400 border-2 border-yellow-200 rounded-full shadow-[0_0_10px_#facc15] animate-pulse dot"; 
          } else {
      ...
      function showVictory() {
        ...
        for(let i = 1; i <= 3; i++) {
            if(i <= stars) {
                starsContainer.innerHTML += `<i data-lucide="star" class="w-12 h-12 text-yellow-400 fill-current drop-shadow-[0_0_15px_#facc15] animate-bounce-slight" style="animation-delay: ${i*0.1}s"></i>`;
            } else {
                starsContainer.innerHTML += `<i data-lucide="star" class="w-12 h-12 text-sky-200 fill-current drop-shadow-sm"></i>`;
            }
        }
        lucide.createIcons();
        ...
        flawlessBadge.style.display = isFlawlessOverall ? 'flex' : 'none';
        
        document.getElementById('earned-xp-text').innerText = `+${levelEarnedXP}`;
        ...
        (function frame() {
          confetti({ particleCount: 8, angle: 60, spread: 55, origin: { x: 0, y: 1 }, colors: ['#3b82f6', '#60a5fa', '#facc15', '#4ade80'] });
          confetti({ particleCount: 8, angle: 120, spread: 55, origin: { x: 1, y: 1 }, colors: ['#3b82f6', '#60a5fa', '#facc15', '#4ade80'] });
        ...
        if (currentState === "PLAYING" && !isAnimating) {
            ...
            if (charHovered) {
              ...
              if (hoveredCharId === currentId) {
                ...
                characters.forEach((c) => c.classList.remove("scale-110", "shadow-[0_0_30px_rgba(255,255,255,0.8)]"));
