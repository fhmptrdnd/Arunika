                    ...
                    <tr class="hover:bg-blue-50/50 transition-colors group">
                        <td class="p-6">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 bg-[#FEF5D3] rounded-2xl flex items-center justify-center font-bold text-orange border-2 border-white shadow-sm shrink-0">
                                    {{ strtoupper(substr($s->name, 0, 1)) }}
                                </div>
                                <div>
                                    <p class="font-bold text-darkblue">{{ $s->name }}</p>
                                    <p class="text-xs text-gray-400 mt-0.5">Terdaftar: {{ $s->created_at->format('d M Y') }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="p-6">
                            @if($s->kelas)
                                <span class="bg-[#EAF7FC] text-[#12A0D7] px-3 py-1.5 rounded-xl text-sm font-bold border border-blue-50">{{ $s->kelas }}</span>
                            @else
                        ...
                        <td class="p-6">
                            <div class="flex items-center gap-2">
                                <span class="text-orange font-bold text-sm bg-[#FFF4E5] px-2 py-1 rounded-lg">⚡ {{ $s->xp ?? 0 }} XP</span>
                            </div>
                        </td>
                        <td class="p-6 text-center">
                            <button class="text-gray-400 hover:text-[#12A0D7] font-bold px-3 py-1 transition-colors border-none bg-transparent cursor-pointer">Edit</button>
                        </td>
                    </tr>
                    @empty
                    <tr>
    ...
    <div class="bg-white rounded-[30px] p-8 w-full max-w-md relative z-10 shadow-2xl">
        ...
        <form action="{{ route('admin.siswa.store') }}" method="POST" class="flex flex-col gap-4">
            ...
            <div>
                ...
                <input type="text" name="name" required placeholder="Cth: Nadiem Makarim" class="w-full px-5 py-3 rounded-full border-2 border-gray-200 focus:border-[#12A0D7] outline-none">
            </div>
            <div>
                <label class="block text-gray-500 text-sm font-semibold ml-2 mb-1">Username (Untuk Login)</label>
                <input type="text" name="username" required placeholder="Cth: nadiem_123" class="w-full px-5 py-3 rounded-full border-2 border-gray-200 focus:border-[#12A0D7] outline-none">
            </div>
            <div>
                <label class="block text-gray-500 text-sm font-semibold ml-2 mb-1">Password Sementara</label>
                <input type="text" name="password" required value="arunika123" class="w-full px-5 py-3 rounded-full border-2 border-gray-200 focus:border-[#12A0D7] outline-none">
                <p class="text-xs text-gray-400 ml-2 mt-1">Default: arunika123 (bisa diubah manual)</p>
            </div>
            <button type="submit" class="w-full bg-[#12A0D7] text-white font-bold py-3 rounded-full mt-2 hover:bg-blue-600 transition-all">Daftarkan Siswa</button>
        </form>
    </div>
</div> --}}
@endsection
