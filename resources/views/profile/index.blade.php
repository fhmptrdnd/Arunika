    ...
    <div class="bg-white rounded-[40px] shadow-2xl w-full p-10 flex flex-col md:flex-row gap-10 animate-pop-in opacity-0 delay-200">
        ...
        <div class="w-full md:w-1/3 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-gray-100 pb-8 md:pb-0">
            ...
            @if($user->role === 'admin')
                ...
                <span class="bg-blue-100 text-[#12A0D7] font-bold px-4 py-2 rounded-full text-sm tracking-widest uppercase">Admin Sekolah</span>
            @else
                <div class="w-40 h-40 flex items-center justify-center mb-6">
                    <img src="{{ asset('images/OrangTua.png') }}" alt="Ortu">
                </div>
                <span class="bg-yellow-100 text-orange font-bold px-4 py-2 rounded-full text-sm tracking-widest uppercase">Orang Tua</span>
            @endif
                    ...
                    @else
                        <form action="{{ route('profile.update.school') }}" method="POST" class="mt-2">
                            ...
                            <div class="flex gap-2">
                                ...
                                <button type="submit" class="bg-[#12A0D7] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-blue-600 transition">
                                    Gabung
                                </button>
                            </div>
                            @error('class_code') <p class="text-edu-red text-xs mt-1 font-bold">{{ $message }}</p> @enderror
                        </form>
                    @endif
                </div>
